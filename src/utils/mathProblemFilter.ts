...
export function isMathProblem(message: string): boolean {
  ...
  const hasMathOperators =
    /[\+\-\*\/×÷=<>≤≥]|bằng|chia|nhân|cộng|trừ|tổng|hiệu|tích|thương/.test(text)
  ...
  const hasProblemStructure =
    /(con|có|mua|cho|nhận|được|mất|bớt|thêm)\s+\d+/.test(text.toLowerCase())
...
export function extractMathProblem(message: string): string {
  ...
  text = text.replace(
    /\s*(cảm ơn|thanks|thank you|cảm ơn cô|cảm ơn nhiều).*$/i,
    ''
  )
...
export function extractMathProblemFromAssistant(message: string): string {
  ...
  for (let i = 0; i < sentences.length; i++) {
    ...
    if (
      /^(Con muốn|Tốt lắm|Cô sẽ cho|Cô Mây|Chào con)/i.test(sentence) &&
      !/\d+/.test(sentence)
    ) {
      continue
    }
    ...
    const hasMathKeywords =
      /(có|cái|viên|quả|người|bạn|bài toán|tính|tìm|chia|nhân|cộng|trừ|bằng|bao nhiêu|gợi ý)/i.test(
        sentence
      )
    ...
    if (hasNumber && hasMathKeywords) {
      ...
      if (
        /(Con có|Mẹ có|Bạn có|Có)\s+\d+\s+(cái|viên|quả|người|bạn|kg|mét|cm|km)/i.test(
          sentence
        )
      ) {
        problemStartIndex = i
  ...
  const mathSentences = sentences.filter(
    (s) =>
      /\d+/.test(s) &&
  ...
  if (mathSentences.length > 0) {
    ...
    const firstMathIndex = sentences.findIndex((s) =>
      s.includes(firstMathSentence)
    )
